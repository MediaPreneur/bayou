...
from bayou.experiments.low_level_sketches.data_reader import Reader
...
{                                         |
    ...
    "decoder": {                          | Provide parameters for the decoder here
        ...
        "max_tokens": 32                  | Maximum number of tokens in the program/AST
    }                                     |
}                                         |
"""
    ...
    with tf.Session() as sess:
        ...
        for i in range(config.num_epochs):
            ...
            for b in range(config.num_batches):
                ...
                # setup the feed dict
                ev_data, t, y = reader.next_batch()
                feed = {model.targets: y}
                for j, ev in enumerate(config.evidence):
                    feed[model.encoder.inputs[j].name] = ev_data[j]
                for j in range(config.decoder.max_tokens):
                    feed[model.decoder.tokens[j].name] = t[j]
